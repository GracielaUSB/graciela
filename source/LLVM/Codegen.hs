...
astToLLVM :: [String] -> AST -> String -> Module
astToLLVM files (AST _ _ _ (Program name defs accs)) version =
  defaultModule
...
createLLVM :: [String] -> [AST] -> AST -> LLVM ()
...
createDef :: AST -> LLVM ()
createDef (AST _ _ _ ast') = case ast' of 
  DefProc name st accs pre post decs params -> do
  ...
  DefFunc fname st exp reType params -> do
